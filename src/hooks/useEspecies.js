...
export function useEspecies(initialParams = {}) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (cancelled) return
        setError(err.response?.data?.message || 'Error al cargar especies')
      } finally {
  ...
  const getById = useCallback(async (id) => {
    ...
    setError(null)
    try {
      const res = await getEspecie(id)
      return res.data.data
    } catch (err) {
      setError(err.response?.data?.message || 'Error al obtener especie')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const create = useCallback(async (formData) => {
    setError(null)
    try {
      const res = await createEspecie(formData)
      setRefreshCount((c) => c + 1)
      return res.data.data
    } catch (err) {
      const msg = err.response?.data?.message || 'Error al crear especie'
      setError(msg)
      throw err
    }
  }, [])
  ...
  const update = useCallback(async (id, formData) => {
    setError(null)
    try {
      const res = await updateEspecie(id, formData)
      setRefreshCount((c) => c + 1)
      return res.data.data
    } catch (err) {
      const msg = err.response?.data?.message || 'Error al actualizar especie'
      setError(msg)
      throw err
    }
  }, [])
  ...
  const remove = useCallback(async (id) => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || 'Error al eliminar especie'
      setError(msg)
      throw err
    }
  }, [])
