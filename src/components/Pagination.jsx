  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6 px-1">
      ...
      <div className="text-sm font-medium text-neutral-muted">
        Mostrando página <span className="font-semibold text-neutral-text">{activePage}</span> de{' '}
        <span className="font-semibold text-neutral-text">{finalTotalPages}</span>
        {finalTotalDocs > 0 && (
          <> (<span className="font-semibold text-neutral-text">{finalTotalDocs}</span> registros en total)</>
        )}
      </div>

      {/* Controles de navegación */}
      <div className="inline-flex rounded-lg border border-neutral-200 bg-neutral-card shadow-sm p-1 gap-1">
        <button
          type="button"
          onClick={() => onPageChange && onPageChange(activePage - 1)}
          disabled={activePage === 1}
          className="inline-flex items-center rounded-md bg-secondary-500 px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-secondary-600 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-secondary-500"
        >
